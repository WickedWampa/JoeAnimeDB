...
check('Beta 12 version identity is consistent across platforms', async () => {
  ...
  assert.equal(packageMetadata.version, '5.0.0-beta.12');
  assert.match(androidSource, /versionCode\s+5000012/);
  assert.match(androidSource, /versionName\s+"5\.0\.0-beta\.12"/);
...
if (failures) {
  console.error(`\nBeta 12 release gate failed: ${failures} check(s).`);
  process.exitCode = 1;
} else {
  console.log(`\nBeta 12 automated release gate passed: ${checks.length} checks.`);
}
