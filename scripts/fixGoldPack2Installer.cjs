...
const pack2Path = path.join(root, 'src', 'ai', 'genome', 'gold', 'goldStandardGenomeCardsPack2.js');
const registryPath = path.join(root, 'src', 'ai', 'genome', 'genomeRegistry.js');
...
console.log('');
console.log('========================================');
console.log(' JoeAnimeDB Gold Pack 2 Fix Installer');
console.log('========================================');
console.log('Repo root:', root);
console.log('');

if (!fs.existsSync(builderPath)) {
  console.error('ERROR: Could not find scripts/rebuildGenomeRegistry.cjs');
  console.error('Make sure you run this from inside the JoeAnimeDB repo folder.');
  process.exit(1);
}

if (!fs.existsSync(pack2Path)) {
  console.error('ERROR: Could not find Gold Pack 2 here:');
  console.error('  ' + rel(pack2Path));
  console.error('Move goldStandardGenomeCardsPack2.js into src/ai/genome/gold/ and run this again.');
  process.exit(1);
}

const backupPath = builderPath + '.backup-before-gold-pack-2';
if (!fs.existsSync(backupPath)) {
  fs.copyFileSync(builderPath, backupPath);
  console.log('Backup created:', rel(backupPath));
} else {
  console.log('Backup already exists:', rel(backupPath));
}

const newBuilder = String.raw`const fs = require('fs');
const path = require('path');
...
// Priority order matters. Gold comes first so curated cards win duplicate IDs.
const sources = [
  path.join(genomeDir, 'gold'),
...
packFiles.forEach((file, index) => {
  ...
  if (!exports.length) {
    warnings.push('No export detected in ' + path.relative(root, file).replaceAll(path.sep, '/'));
    return;
  }
...
console.log('Registry file: ' + path.relative(root, registryFile).replaceAll(path.sep, '/'));
console.log('');
...
fs.writeFileSync(builderPath, newBuilder, 'utf8');
console.log('Replaced registry builder with Gold-aware version:', rel(builderPath));

console.log('Rebuilding genome registry...');
const rebuild = spawnSync(process.execPath, [builderPath], { cwd: root, stdio: 'inherit' });
if (rebuild.status !== 0) {
  console.error('ERROR: Registry rebuild failed.');
  process.exit(rebuild.status || 1);
}

if (!fs.existsSync(registryPath)) {
  console.error('ERROR: Registry file was not created:', rel(registryPath));
  process.exit(1);
}

const registryText = fs.readFileSync(registryPath, 'utf8');
if (!registryText.includes('goldStandardGenomeCardsPack2')) {
  console.error('ERROR: Registry rebuilt, but Pack 2 was not detected in genomeRegistry.js');
  process.exit(1);
}

console.log('SUCCESS: Gold Pack 2 is now wired into the registry.');
console.log('');
