...
const packFiles = sources
  ...
  .sort((a, b) => {
    ...
    const priority = (rel) => {
      if (rel.includes('/gold/')) return 0;
      if (rel.includes('/core25/')) return 1;
      if (rel.includes('/enhanced/')) return 2;
      if (rel.includes('/core100/')) return 3;
      if (rel.includes('/generated/')) return 4;
      if (rel.includes('/modules/')) return 5;
      return 9;
    };
...
packFiles.forEach((file, index) => {
  ...
  for (const exportName of exports) {
    spreads.push(`  ...normalizePack(${importName}.${exportName}, '${relFromRoot(root, file)}#${exportName}'),`);
  }
});
...
console.log('');
console.log('========================================');
console.log(' JoeAI Genome Registry Builder v0.2.1');
console.log('========================================');
...
console.log(goldPack2Detected ? '✅ Gold Pack 2 detected in registry.' : '⚠️  Gold Pack 2 was not detected. Check that the file is in src/ai/genome/gold.');
